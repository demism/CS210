...
public class CityMonthlyRainListImpl {
  ...
  /**
    ...
    * (or modify the rainfall measurement of an existing record)
    * 
   ...
   public void insertMonthsRainFallRecord(int month, int year, double rain) {
      ...
      while ( loop )
      {
    	  ...
    	  else if ( (count != monthlyRainList.size()-1) &&(tempRF.getYear() == year ) && (tempRF.getMonth() < month) && (month < monthlyRainList.get(count + 1).getMonth()))
    	  { // middle spot 
    		  System.out.printf("%.2f inches of rain recorded for %02d/%d\n",rain,month,year);
    		  monthlyRainList.add(count+1, new RainFall(month,year,rain));
    		  loop = false;
    	  }
    	  else if ( ( year < tempRF.getYear() ) || ( (year == tempRF.getYear()) && (month < tempRF.getMonth()) ) )  
    	  { // First Spot
    		  System.out.printf("%.2f inches of rain recorded for %02d/%d\n",rain,month,year);
    		  monthlyRainList.add(count, new RainFall(month,year,rain));
    		  loop = false;
    	  }
    	  else if (count == monthlyRainList.size()-1) 
    	  {	// Final spot
    		  System.out.printf("%.2f inches of rain recorded for %02d/%d\n",rain,month,year);
    		  monthlyRainList.add(new RainFall(month,year,rain));
    		  loop = false;
    	  }
    	  else
    		  count++;	  
